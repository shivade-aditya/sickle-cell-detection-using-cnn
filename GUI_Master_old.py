...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        model = load_model('cell_model.h5',compile=False)
        ...
        if cell == 0:
            Cd="Sickel Cell Detected \n Medicine: Hydroxyurea \nMaintaining personal hygiene \n Avoid uncooked meats, fish, frogs, snails, slugs, freshwater ."
            
        elif cell == 1:
            Cd="LYMPHOCYTE Cell Detected\n Medicine: cyclosporine, tacrolimus\nWhite blood cells called lymphocytes increase with these conditions\nProtein foods include egg whites, beans, fish, chicken "
            
        elif cell == 2:
            Cd="MONOCYTE Cell Detected \n Medicine: leukocytes\n Avoiding foods that cause inflammation like red meats, refined carbohydrates\n Include Green, leafy vegetables such as kale, spinach"
            
        elif cell == 3:
            Cd="NEUTROPHIL Cell Detected \n Medicine: Filgrastim\n Include practicing good hygiene and avoiding food that might have germs\n Seek medical help immediately for a fever."
      
            
        A=Cd
        return A
    img = open(fn)    
    myfile = open("C:/Users/TEJAS/Downloads/Code/Blood Cell Identification 100% Code/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='C:/Users/tejas/OneDrive/Desktop/Sickel cell detection/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
